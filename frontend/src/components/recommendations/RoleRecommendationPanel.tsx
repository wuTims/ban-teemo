// frontend/src/components/recommendations/RoleRecommendationPanel.tsx
...
function RolePickCard({
  ...
  return (
    <button
      ...
      className="
        bg-lol-darkest/50 rounded p-1.5 sm:p-2 border border-gold-dim/30
    ...
    >
      ...
      <div className="flex items-center gap-1.5 sm:gap-2">
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-center justify-between gap-1">
            <span className="text-[10px] sm:text-xs font-medium text-gold truncate">
              {pick.champion_name}
            </span>
            <span className={`text-[10px] sm:text-xs font-bold ${scoreColor} shrink-0`}>
              {Math.round(pick.score * 100)}%
            </span>
          </div>
          {playerName && (
            <div className="text-[9px] sm:text-[10px] text-text-tertiary truncate">
              {playerName}
            </div>
          )}
        </div>
      ...
      {/* Component breakdown - compact inline format, hidden on mobile */}
      {topFactors.length > 0 && (
        <div className="hidden sm:flex mt-1.5 gap-1 flex-wrap">
          {topFactors.map((factor) => (
            <span
              key={factor.name}
              className={`
                text-[9px] sm:text-[10px] px-1 py-0.5 rounded bg-lol-dark/50
    ...
    ">
      ...
      {isExpanded && (
        <div className="px-2 sm:px-3 pb-2 sm:pb-3 pt-1">
          {/* Responsive grid: 2 cols on mobile (with wrapping), 5 on lg+ */}
          <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-2">
            {ROLE_ORDER.map((role) => {
              ...
              return (
                <div key={role} className="flex flex-col gap-1 sm:gap-1.5">
                  {/* Role Header */}
                  <div className="text-center">
                    <span className="text-[10px] sm:text-[11px] font-semibold uppercase tracking-wide text-gold-dim">
                  ...
                  {picks.length > 0 ? (
                    ...
                    <div className="
                      ...
                      py-3 sm:py-4 bg-lol-darkest/30 rounded
                    ">
                      Filled
                    </div>
                  )}
